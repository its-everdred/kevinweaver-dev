...
vi.mock('../../lib/viz/render/budget', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../lib/viz/render/budget')>()
  return {
    ...actual,
...
const RIBBON_GEOMETRY = {
  cssWidth: 530,
  cssHeight: 140,
  dpr: 1,
  pxWidth: 530,
  pxHeight: 140,
  fontPx: { micro: 7, small: 9, mono: 13 },
...
test('renders a ribbon using attached device geometry and fonts', async () => {
  ...
  driver.attach({ id: 'ribbon', ctx: recorded.ctx, geometry: RIBBON_GEOMETRY })
  ...
  expect(
    recorded.calls.some(
      ([name, value]) => name === 'set:font' && value === '600 7px Test Mono'
    )
  ).toBe(true)
  expect(driver.inspect().highlightCell).toEqual(highlightCellFor(INPUT, 31, 31))
...
test('replaces and detaches a surface context', async () => {
  ...
  driver.attach({ id: 'ribbon', ctx: first.ctx, geometry: RIBBON_GEOMETRY })
  await driver.renderFrame(0)
  expect(first.calls.some(([name]) => name === 'fillRect')).toBe(true)

  const firstCallsBeforeReplacement = first.calls.length
  driver.attach({ id: 'ribbon', ctx: second.ctx, geometry: RIBBON_GEOMETRY })
  await driver.renderFrame(0)
  ...
  await driver.renderFrame(0)
  expect(second.calls).toHaveLength(callsBeforeDetach)
...
test('coalesces invalidation into the dirty surface paint', () => {
  ...
  driver.attach({ id: 'ribbon', ctx: ribbon.ctx, geometry: RIBBON_GEOMETRY })
  driver.attach({ id: 'overview', ctx: overview.ctx, geometry: overviewGeometry })
  runPaint?.(0)
