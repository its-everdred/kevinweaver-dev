...
| **D-15** | **Phone number 856-723-2521 never enters the repo or build output.** Email decision deferred to HG-5. | content-ia's SIM-swap analysis, confirmed by its verifier. `kevinweaver.dev` already has five live Namecheap MX records, so an alias is a registrar-panel row, not a new service. | Obfuscation (does not survive OCR); publishing as-is. |
...
*Acceptance:* `grep -r '856-723-2521' .` is empty including the build output; no contribution integer literal in `content/`; every employer from the authoritative resume present; all SHAs valid hex with no shared prefixes.
