...
import { highlightVizRibbonPointer, syncVizRibbonWindow } from './surface-pointer'
import { createVizSurfaceInvalidations } from './surface-invalidation'
import type {
  VizPointer,
  VizSurfaceAdapter,
...
export function createVizDriver(options: VizDriverOptions): VizDriver {
  ...
  const surfaceViews = createVizSurfaceViews()
  const invalidations = createVizSurfaceInvalidations()
  ...
  function scheduleInvalidatedPaint(): void {
    if (running || invalidationRaf || typeof requestAnimationFrame !== 'function')
      return
    invalidationRaf = requestAnimationFrame(() => {
      invalidationRaf = 0
      const targets = invalidations.drain()
  ...
  function paint(
    ...
    if (overview && (targets === undefined || targets.has('overview')))
      renderOverview(
        state,
        overview,
        buildRenderView('overview'),
        layers.overview,
        winStart
      )
    const ribbon = contexts.get('ribbon')
    const ribbonView = buildRenderView('ribbon')
    const paintsRibbon = targets === undefined || targets.has('ribbon')
    if (ribbon && paintsRibbon)
      renderRibbon(state, ribbon, ribbonView, layers.ribbon)
    if (ribbonPointer && paintsRibbon)
      pointerHighlight = ribbonHighlight(ribbonPointer, ribbonView)
    const graph = contexts.get('graph')
    if (graph && (targets === undefined || targets.has('graph')))
      renderGraph(state, graph, buildRenderView('graph'), layers.graph)
    const report = layers.budget.end()
    if (targets === undefined) invalidations.clear()
