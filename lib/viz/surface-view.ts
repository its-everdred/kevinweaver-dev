...
function renderViewport(
  ...
  return {
    ...viewport,
    pxWidth: geometry?.pxWidth ?? Math.max(1, Math.round(viewport.cssWidth * viewport.dpr)),
    pxHeight: geometry?.pxHeight ?? Math.max(1, Math.round(viewport.cssHeight * viewport.dpr)),
  }
}
...
function surfaceTheme(geometry: VizSurfaceGeometry | undefined): RenderTheme {
  return {
    ...
    token: {
      bgH: PANE_SURFACE,
      bg0: LV[0], bg1: LV[1], bg2: LV[2], bg3: LV[3], bg4: LV[4],
      fg0: LV[9], fg1: LV[8], fg2: LV[7], fg3: LV[6], fg4: LV[5],
      gray: LV[4], green: LV[8], greenD: LV[6], aqua: AG[8], aquaD: AG[6],
      purple: AG[7], purpleD: AG[5], yellow: LV[9], yellowD: LV[7],
      red: AG[8], blue: AG[7],
    },
    fontPx: geometry?.fontPx ?? FALLBACK_FONT,
    fontFamily: geometry?.fontFamily ?? 'monospace',
  }
}
